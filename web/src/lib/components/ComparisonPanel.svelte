<script lang="ts">
  import { getJobResults, getJobMetrics, type Job, type JobResult, type JobMetrics, type WindowMetrics } from "$lib/api";
  ...
  $effect(() => {
    for (const job of jobs) {
      ...
      Promise.all([getJobResults(job.id), getJobMetrics(job.id)])
        ...
        .catch((e) => {
          jobData[job.id] = { results: null, metrics: null, figures: [], error: e.message, loading: false };
        });
    }
  });
  ...
  const allWindows = $derived(
    [...new Set(
      jobs.flatMap((j) => jobData[j.id]?.metrics?.windows.map((w) => w.window) ?? [])
    )].sort()
  );
    ...
    [...new Map(
      jobs.flatMap((j) =>
        (jobData[j.id]?.figures ?? [])
          ...
          .map((f) => [`${f.kind}|${f.window}`, { kind: f.kind, window: f.window, label: `Spatial — Days ${f.window}` }])
      )
    ).values()].sort((a, b) => (a.window ?? "").localeCompare(b.window ?? ""))
  );
                ...
                <tr>
                  ...
                  {#each jobs as job, i}
                    <th class="job-col">
                      ...
                      <span class="job-col-dates">{job.params?.start_date ?? ""} – {job.params?.end_date ?? ""}</span>
                    </th>
                        ...
                        <td class="delta">
                          {#if s && baseStat}
                            ...
                            <span class={deltaClass(d)}>
                              {d >= 0 ? "+" : ""}{s.unit === "fraction" ? (d * 100).toFixed(1) + "%" : d.toFixed(1) + " d"}
                            </span>
                          {:else}
                            <span class="na">—</span>
                          {/if}
      ...
      <div class="fig-grid" style="--ncols: {jobs.length}">
        ...
        {#each figurePairs as pair}
          ...
          {#each jobs as job, ji}
            ...
            <div class="fig-cell" onclick={() => fig && openLightbox(figs, figs.indexOf(fig))} title={fig ? "Click to expand" : ""}>
              {#if jobData[job.id]?.loading}
...
<style>
  ...
  /* ── Metrics table ── */
  .win-block { display: flex; flex-direction: column; gap: 0.4rem; }
  ...
  th.job-col { min-width: 110px; }
  th.delta-col { min-width: 70px; color: var(--color-text-dim); opacity: 0.7; }
  ...
  td.delta { font-size: 0.73rem; }

  .delta-better { color: #4caf78; }
  .delta-worse  { color: #e07070; }
  .delta-neutral { color: var(--color-text-dim); }

  .na { color: var(--color-text-dim); opacity: 0.5; }
  .loading-dot { color: var(--color-text-dim); font-style: italic; }

  tbody tr:last-child td { border-bottom: none; }
  tbody tr:hover td { background: var(--color-accent-glow); }
  ...
  .fig-cell img { width: 100%; display: block; }
  ...
  .cell-placeholder.na { font-style: italic; opacity: 0.6; }
  .cell-placeholder.error { color: var(--color-danger); }
